...
- オプション `-nonutf8` を実装: UTF8 だと判定させない
- オプション `-help` を実装: オプション一覧を表示する
- オプション `-semicolon` を実装: セミコロンを区切り文字にする
- オプション `-auto "string"` を実装：一連のキー操作を`|`でつないで指定する
- オプション `-16le`, `-16be` を実装：強制的に UTF16LE,UTF16BE と判断させる
