...
- 起動時に曖昧幅の Unicode 文字の表示幅を計測していたが、カーソル位置を取得するシーケンス `ESC[6n` をサポートしない端末では、入力待ちのままエラー終了してしまう問題があった。そのため、幅を明示的に指定して計測をスキップできる `-aw`（曖昧幅を2セルとして扱う）および `-an`（曖昧幅を1セルとして扱う）オプションを追加した。
