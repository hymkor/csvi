...
デフォルトでは、エディターは入力ファイルから改行コードを検出します。
入力データが与えられなかった場合、OS のデフォルト改行コードを使用しますが、
これらのオプションで変更可能です。

* `-lf` 新規追加行に対する改行コードとして LF(`\n`) を使用します
* `-crlf` 新規追加行に対する改行コードとして CRLF(`\r\n`) を使用します

これらのオプションは排他で、同時に使用するとエラーになります。
...
* 編集
    ...
    * `Meta`+`p` (現在のセル/列/行を内部クリップボードの値で上書き)
    * `s` (ヘッダーより下の行を、現在の列の値で昇順にソート)
    * `S` (ヘッダーより下の行を、現在の列の値で降順にソート)
* 表示設定
